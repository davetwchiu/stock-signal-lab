...
def _stress_relative_strength_ticker_rows(
    ...
    *,
    ...
    downside_capture = ticker_down / benchmark_down.mask(benchmark_down.eq(0))
    day_range = pd.to_numeric(data["High"], errors="coerce") - pd.to_numeric(data["Low"], errors="coerce")
    close_position = ((pd.to_numeric(data["Close"], errors="coerce") - pd.to_numeric(data["Low"], errors="coerce")) / day_range.mask(day_range.eq(0)))
